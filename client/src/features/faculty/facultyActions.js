...
export const getFacultyDetails = (facultyId) => async (dispatch) => {
  try {
    ...
    const { data } = await axiosInstance.get(`/api/faculties/${facultyId}`);
...
export const createFaculty = (facultyData) => async (dispatch) => {
  try {
    ...
    const { data } = await axiosInstance.post("/api/faculties", facultyData);
...
export const updateFaculty = (facultyId, facultyData) => async (dispatch) => {
  try {
    dispatch(updateFacultyRequest());

    facultyData.append("_method", "PATCH");
...
export const deleteFaculty = (facultyId) => async (dispatch) => {
  try {
    ...
    const { data } = await axiosInstance.delete(`/api/faculties/${facultyId}`);
