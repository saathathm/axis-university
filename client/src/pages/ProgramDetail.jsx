...
import { fetchProgramById } from "@/actions/programAction";
...
const ProgramDetail = () => {
  ...
  const apiPrograms = useSelector((state) => state.program.programs);
  const selectedProgram = useSelector((state) => state.program.selectedProgram);
  ...
  const program = useMemo(() => {
    return (
      selectedProgram ||
      apiPrograms.find((item) => String(item.id) === String(programId))
    );
  }, [apiPrograms, programId, selectedProgram]);

  if (!program) {
    return <ProgramNotFound />;
  }

  const facultyName = program.facultyName || "Academic Program";
  const curriculum = getCurriculum(program);
  const requirements = program.requirements?.length
